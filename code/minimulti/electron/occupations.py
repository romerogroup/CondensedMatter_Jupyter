    ...
    for i in range(ni):
        for j in range(nj):
            ...
            if x < -10:
                ...
                ret[i, j] = 2.0 / nspin / (math.exp(x) + 1)
    return ret


#myfermi=np.vectorize(myfermi_s)
...
class Occupations(object):
    ...
    def fermi(self, mu):
        ...
        #return (2 / self.nspin) / (np.exp(args) + 1.0)
    ...
    def root_function(self, mu):
        ...
        return np.einsum('j, ij->', self.wk, f) - self.nel
        #return np.dot(f, self.wk).sum()-self.nel
    #@profile
